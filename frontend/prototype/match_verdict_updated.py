...
def _roster(conn, match_id, team_id):
    ...
    starter off the list. Rating is averaged across whatever maps they did
    play, side='both' rows only (see schema note on the side dimension)."""
    cur = conn.execute(
        """SELECT p.name, AVG(s.rating) AS avg_rating, COUNT(*) AS maps_played
    ...
    return [{"name": name, "rating": round(rating, 2)} for name, rating, _ in cur.fetchall()]
